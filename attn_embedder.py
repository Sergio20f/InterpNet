...
class AttnEmbedder(nn.Module):
    """
    In essence, this class converts the weights of a convolutional network into an embedded sequence, applies a 
    transformer to them, and outputs a transformed representation of these weights. The expected input to this 
    class would be the weights of a CNN, and the output would be the embedded, position-encoded, and attention-
    transformed representation of these weights.
    """
    def __init__(self, index_dict, d_model, n_heads, d_embed, seed=22):
        ...
        tra_norm = None
        self.transfomer = nn.TransformerEncoder(encoder_layer=encoder_layer, num_layers=self.N, norm=tra_norm)
    ...
    def get_max_weights_per_neuron(self):
        weights_list = []
    ...
    def get_kernel_slices(self): # Is this only valid for CNNs?
        slice_lst = []
        # loop over layers
        for idx, layer in enumerate(self.index_dict["layer"]):
            kernel_slice_lst = []
